...
def Rmm(a, b, func, m, *args):
    ...
    for k in range(1,m+1):
        ...
        for i in range(1, 2**(k-1)+1):
            trapzd_sum += func(a + (2*i-1)*hk[k], *args)
...
def romberg(func, a, b, rtol = 1.e-4, mmax = 12, verbose = False, args = []):
    """
...
def age_func(a, Om0, OmL):
    return 1./(a*np.sqrt(Om0/a**3 + OmL + (1.-Om0 - OmL)/a**2))

def age(a, *args): 
    """
    function computing dimensionless age of the universe
    to get actual age, multiply output by 1/H0 
    a = expansion factor
    args[0] = Om0
    args[1] = OmL
    """
    return (romberg(age_func, 1.e-9, a, rtol=1.e-10, args=args)[0])